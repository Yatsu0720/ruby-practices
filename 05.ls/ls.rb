...
def fetch_file_list
  ...
  file_list.each_with_index do |file, integer|
    ...
    number_of_hard_link[integer] = file_state.nlink
    user_name[integer] = Etc.getpwuid(file_state.uid).name
    group_name[integer] = Etc.getgrgid(file_state.gid).name
    file_size[integer] = file_state.size
    timestamp[integer] = file_state.mtime.strftime('%b %d %R')
